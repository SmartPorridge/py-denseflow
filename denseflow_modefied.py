...
def save_flows(flows,image,save_dir,num,bound):
    ...
    if not os.path.exists(os.path.join(flow_root,new_dir,save_dir)):
        os.makedirs(os.path.join(flow_root,new_dir,save_dir))

    #save the image
    save_img=os.path.join(flow_root,new_dir,save_dir,'img_{:05d}.jpg'.format(num))
    #scipy.misc.imsave(save_img,image)

    #save the flows
    save_x=os.path.join(flow_root,new_dir,save_dir,'flow_x_{:05d}.jpg'.format(num))
    save_y=os.path.join(flow_root,new_dir,save_dir,'flow_y_{:05d}.jpg'.format(num))
...
def dense_flow(augs):
    ...
    :return: no returns
    zip(video_list,flows_dirs,[step]*len(video_list),[bound]*len(video_list))
    '''
    video_name,save_dir,step,bound=augs
    video_path=os.path.join(videos_root,video_name)
    if (not os.path.exists(os.path.join(exist_flow_root_1,new_dir,save_dir))) and (not os.path.exists(os.path.join(exist_flow_root_2,new_dir,save_dir))):
        ...
        print("{} already processed,next……".format(video_name))


def get_video_list():
    video_list=[]
    for cls_name in os.listdir(videos_root):
        cls_path=os.path.join(videos_root,cls_name)
        for video_ in os.listdir(cls_path):
            video_list.append(os.path.join(cls_name,video_))
    video_list.sort()
...
def parse_args():
    ...
    parser.add_argument('--new_dir',default='TVL1_flow/train_112',type=str)
    parser.add_argument('--num_workers',default=40,type=int,help='num of workers to act multi-process')
...
if __name__ =='__main__':
    ...
    data_root='/root/to/your/path/'
    data_root=os.path.join(data_root,dataset)

    flow_root = '/path/to/Moments_in_Time_Raw/'

    exist_flow_root_1 =  ''
    new_dir = 'TVL1_flow/train_112'

    args=parse_args()
    #data_root=os.path.join(args.data_root,args.dataset)
    videos_root=os.path.join(data_root,'video/train_112')
    ...
    #read from file list
    video_list = []
    video_list_file = open("list_part_1.list",'r')
